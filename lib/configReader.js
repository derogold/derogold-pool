...
// Environment overrides make the same config usable when dependencies live on
// a Docker network instead of localhost.
function override (environmentName, section, property, parser) {
  if (process.env[environmentName] === undefined) return
  global.config[section][property] = parser
    ? parser(process.env[environmentName])
    : process.env[environmentName]
}

override('REDIS_HOST', 'redis', 'host')
override('REDIS_PORT', 'redis', 'port', Number)
override('DAEMON_HOST', 'daemon', 'host')
override('DAEMON_PORT', 'daemon', 'port', Number)
override('WALLET_HOST', 'wallet', 'host')
override('WALLET_PORT', 'wallet', 'port', Number)
override('WALLET_DAEMON_HOST', 'wallet', 'daemonHost')
override('WALLET_DAEMON_PORT', 'wallet', 'daemonPort', Number)
override('API_HOST', 'api', 'host')
override('API_PORT', 'api', 'port', Number)
