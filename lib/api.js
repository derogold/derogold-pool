...
var paymentHealth = require('./paymentHealth.js')
...
function handlePublicHealth (response) {
  ...
  async.parallel({
    monitoring: getMonitoringData,
    payments: getLastPaymentData
  }, function (error, result) {
    if (error) {
      global.log('info', logSystem, 'Could not handle public health: %s', [error])
    }
    response.end(JSON.stringify(buildPublicHealth(result && result.monitoring || {}, result && result.payments || {})))
  })
}
...
function buildPublicHealth (monitoring, payments) {
  ...
  var services = {
    ...
    payments: {
      label: 'DEGO payments',
      status: publicPaymentStatus(monitoring.wallet, global.config.payments, payments.parent, now),
      lastCheck: monitoring.wallet && monitoring.wallet.lastCheck || null,
      lastPayment: payments.parent || null
    }
  }
  ...
  if (mergedMiningConfig.enabled && mergedMiningConfig.child) {
    ...
    services['mergedMining:' + childSymbol + ':payments'] = {
      ...
      status: publicPaymentStatus(monitoring[childWalletKey], childPayments, payments.child, now),
      lastCheck: monitoring[childWalletKey] && monitoring[childWalletKey].lastCheck || null,
      lastPayment: payments.child || null
    }
  }
...
function publicPaymentStatus (walletMonitoring, paymentConfig, lastPayment, now) {
  return paymentHealth.status(
    walletMonitoring && walletMonitoring.lastStatus || 'unknown',
    lastPayment,
    now,
    !paymentConfig || paymentConfig.enabled !== false
  )
}

function getLastPaymentData (callback) {
  var commands = [
    ['zrevrange', global.config.coin + ':payments:all', 0, 0, 'WITHSCORES']
  ]
  if (mergedMiningConfig.enabled && mergedMiningConfig.child) {
    commands.push(['zrevrange', getChildPrefix() + ':payments:all', 0, 0, 'WITHSCORES'])
  }

  global.redisClient.multi(commands).exec(function (error, replies) {
    if (error) return callback(error)
    callback(null, {
      parent: paymentHealth.lastPaymentScore(replies && replies[0]),
      child: paymentHealth.lastPaymentScore(replies && replies[1])
    })
  })
}
