...
function handleAdminStats (response) {
  ...
  Promise.all(contexts.map(collectAdminCoinStats)).then(function (coinStats) {
    ...
    response.end(JSON.stringify(responseStats))
...
function normalizeMarketPrices (prices, symbols) {
  var displayAmount = Number(marketConfig.displayAmount || 1000000)
  if (!Number.isFinite(displayAmount) || displayAmount <= 0) displayAmount = 1000000
