...
function parseBlockList (blocks) {
  ...
  for (var i = 0; i < blocks.length; i += 2) {
    ...
    parsedBlocks.push({
      ...
      difficulty: toNumberOrNull(parts[2]),
      shares: toNumberOrNull(parts[3]),
...
function parseSubmittedChildBlocks (blocks) {
  ...
  for (var i = 0; i < blocks.length; i += 2) {
    ...
    parsedBlocks.push({
      ...
      timestamp: toNumberOrNull(parts[1]),
      difficulty: toNumberOrNull(parts[2]),
      shareDifficulty: toNumberOrNull(parts[3]),
      status: parts[4] || null,
      height: height
    })
  }
...
function parseCandidateChildBlocks (blocks) {
  ...
  for (var i = 0; i < blocks.length; i += 2) {
    ...
    parsedBlocks.push({
      ...
      difficulty: toNumberOrNull(parts[2]),
      shares: toNumberOrNull(parts[3]),
      shareDifficulty: toNumberOrNull(parts[4]),
      status: parts[5] || null,
      height: height
    })
  }
...
function parseMaturedChildBlocks (blocks) {
  ...
  for (var i = 0; i < blocks.length; i += 2) {
    ...
    parsedBlocks.push({
      ...
      orphaned: parts[4] !== undefined ? parts[4] === '1' : null,
      reward: toNumberOrNull(parts[5]),
      shareDifficulty: toNumberOrNull(parts[6]),
      status: parts[7] || null,
      height: height
    })
  }
  ...
  if (results.mergedMining && results.mergedMining.enabled && results.mergedMining.child) {
    ...
    coins[childSymbol] = {
      ...
      pool: {
        ...
        minimumPayout: mergedMiningConfig.child.minimumPayout !== undefined ? mergedMiningConfig.child.minimumPayout : null,
        paymentInterval: mergedMiningConfig.child.paymentInterval !== undefined ? mergedMiningConfig.child.paymentInterval : null
      },
      blocks: {
...
function initMonitoring () {
  var modulesRpc = {
    ...
    wallet: function (method, params, callback) {
