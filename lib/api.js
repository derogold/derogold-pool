...
function effectiveMinimumPayout (paymentConfig, address, addressPrefix) {
  var minPayment = paymentConfig && paymentConfig.minPayment !== undefined ? Number(paymentConfig.minPayment) : null
  var minPaymentIdPayment = paymentConfig && paymentConfig.minPaymentIdPayment !== undefined ? Number(paymentConfig.minPaymentIdPayment) : null
  if (!address || !addressPrefix || !isFinite(minPayment) || !isFinite(minPaymentIdPayment)) return minPayment

  try {
    var decoded = turtleUtil.decodeAddress(address, addressPrefix)
    if (decoded && decoded.paymentId && minPaymentIdPayment > minPayment) {
      return minPaymentIdPayment
    }
  } catch (e) {}
...
function childAddressPrefix () {
  if (!mergedMiningConfig.enabled || !mergedMiningConfig.child || !mergedMiningConfig.child.poolAddress) return null
...
function buildMinerCoinStats (address, parentStats, parentPayments, childReplies) {
  ...
  coins[global.config.symbol] = {
    ...
    effectiveMinimumPayout: effectiveMinimumPayout(global.config.payments, address, addressBase58Prefix),
  ...
  if (mergedMiningConfig.enabled && mergedMiningConfig.child) {
    ...
    coins[childSymbol] = {
      ...
      effectiveMinimumPayout: effectiveMinimumPayout(childPayments, childStats.payoutAddress, childAddressPrefix()),
