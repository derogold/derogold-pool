...
function restRequest (host, port, method, path, data, callback, apiKey) {
  ...
  var req = (port === 443 ? https : http).request(options, function (res) {
    ...
    res.on('end', function () {
      if (!replyData) {
        callback(new Error('Empty response from wallet-api (HTTP ' + res.statusCode + ')'))
        return
      }
      var replyJson
      try {
        ...
        callback(new Error('Invalid JSON from wallet-api (HTTP ' + res.statusCode + '): ' + replyData.slice(0, 200)))
        return
      }
