...
function restRequest (host, port, method, path, data, callback, apiKey) {
  ...
  var req = (port === 443 ? https : http).request(options, function (res) {
    ...
    res.on('end', function () {
      if (!replyData) {
        callback({ message: 'Empty response from wallet-api', statusCode: res.statusCode })
        return
      }
      var replyJson
      try {
        ...
        callback({ message: 'Invalid JSON from wallet-api', statusCode: res.statusCode, body: replyData.slice(0, 200) })
        return
      }
      if (replyJson.error || replyJson.errorCode) {
        callback(replyJson.error || replyJson.errorMessage || replyJson.errorCode)
        return
      }
...
module.exports = function (daemonConfig, walletConfig, poolApiConfig) {
  return {
    ...
    rpcDaemonConfig: function (targetDaemonConfig, method, params, callback) {
      rpc(targetDaemonConfig.host, targetDaemonConfig.port, method, params, callback)
    },
