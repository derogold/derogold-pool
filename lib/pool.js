...
Miner.prototype = {
  ...
  getTargetHex: function () {
    ...
    const diffBuff = bigIntToBuffer(diff1 / BigInt(this.difficulty))
    diffBuff.copy(padded, 32 - diffBuff.length)

    const buff = padded.slice(0, 4)
    const buffArray = Array.from(buff).reverse()
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  const hashArray = Array.from(hash).reverse()
  const hashNum = BigInt('0x' + Buffer.from(hashArray).toString('hex'))
  const hashDiff = diff1 / hashNum
